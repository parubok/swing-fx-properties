...
/**
 * A {@code WeakChangeListener} can be used, if an {@link swingfx.beans.value.ObservableValue}
 ...
 * {@link swingfx.beans.value.ChangeListener}. The {@code WeakChangeListener} should then be
 ...
 * to soon.
 *
 * @see swingfx.beans.value.ChangeListener
 ...
 * @since JavaFX 2.0
 */
public final class WeakChangeListener<T> implements swingfx.beans.value.ChangeListener<T>, WeakListener {

    private final WeakReference<swingfx.beans.value.ChangeListener<T>> ref;
     ...
     */
    public WeakChangeListener(@NamedArg("listener") swingfx.beans.value.ChangeListener<T> listener) {
        if (listener == null) {
            throw new NullPointerException("Listener must be specified.");
        }
        this.ref = new WeakReference<swingfx.beans.value.ChangeListener<T>>(listener);
    }
