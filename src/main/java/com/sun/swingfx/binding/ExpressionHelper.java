...
public abstract class ExpressionHelper<T> extends ExpressionHelperBase {
    ...
    private static class SingleInvalidation<T> extends ExpressionHelper<T> {
        ...
        protected ExpressionHelper<T> addListener(InvalidationListener listener) {
            return new com.sun.swingfx.binding.ExpressionHelper.Generic<T>(observable, this.listener, listener);
        }
    ...
    private static class SingleChange<T> extends ExpressionHelper<T> {
        ...
        @Override
        protected ExpressionHelper<T> addListener(InvalidationListener listener) {
            return new com.sun.swingfx.binding.ExpressionHelper.Generic<T>(observable, listener, this.listener);
        }
    ...
    private static class Generic<T> extends ExpressionHelper<T> {
        ...
        @Override
        protected com.sun.swingfx.binding.ExpressionHelper.Generic<T> addListener(InvalidationListener listener) {
                ...
                for (int index = 0; index < invalidationSize; index++) {
                    if (listener.equals(invalidationListeners[index])) {
                        if (invalidationSize == 1) {
                            if (changeSize == 1) {
                                return new com.sun.swingfx.binding.ExpressionHelper.SingleChange<T>(observable, changeListeners[0]);
                            }
                            invalidationListeners = null;
                            invalidationSize = 0;
                        } else if ((invalidationSize == 2) && (changeSize == 0)) {
                            return new com.sun.swingfx.binding.ExpressionHelper.SingleInvalidation<T>(observable, invalidationListeners[1-index]);
                        } else {
            ...
            if (changeListeners != null) {
                for (int index = 0; index < changeSize; index++) {
                    if (listener.equals(changeListeners[index])) {
                        if (changeSize == 1) {
                            if (invalidationSize == 1) {
                                return new com.sun.swingfx.binding.ExpressionHelper.SingleInvalidation<T>(observable, invalidationListeners[0]);
                            }
                            changeListeners = null;
                            changeSize = 0;
                        } else if ((changeSize == 2) && (invalidationSize == 0)) {
                            return new com.sun.swingfx.binding.ExpressionHelper.SingleChange<T>(observable, changeListeners[1-index]);
                        } else {
