...
import com.sun.swingfx.collections.NonIterableChange;
...
import static swingfx.collections.ListChangeListener.Change;
...
public abstract class ListExpressionHelper<E> extends ExpressionHelperBase {
    ...
    private static class SingleInvalidation<E> extends ListExpressionHelper<E> {
        ...
        protected ListExpressionHelper<E> addListener(InvalidationListener listener) {
            return new com.sun.swingfx.binding.ListExpressionHelper.Generic<E>(observable, this.listener, listener);
        }
    ...
    private static class SingleChange<E> extends ListExpressionHelper<E> {
        ...
        @Override
        protected ListExpressionHelper<E> addListener(InvalidationListener listener) {
            return new com.sun.swingfx.binding.ListExpressionHelper.Generic<E>(observable, listener, this.listener);
        }
                    ...
                    if (listener.equals(invalidationListeners[index])) {
                        if (invalidationSize == 1) {
                            if ((changeSize == 1) && (listChangeSize == 0)) {
                                return new com.sun.swingfx.binding.ListExpressionHelper.SingleChange<E>(observable, changeListeners[0]);
                            } else if ((changeSize == 0) && (listChangeSize == 1)) {
                                return new com.sun.swingfx.binding.ListExpressionHelper.SingleListChange<E>(observable, listChangeListeners[0]);
                            }
                            invalidationListeners = null;
                            invalidationSize = 0;
                        } else if ((invalidationSize == 2) && (changeSize == 0) && (listChangeSize == 0)) {
                            return new com.sun.swingfx.binding.ListExpressionHelper.SingleInvalidation<E>(observable, invalidationListeners[1-index]);
                        } else {
                    ...
                    if (listener.equals(changeListeners[index])) {
                        if (changeSize == 1) {
                            if ((invalidationSize == 1) && (listChangeSize == 0)) {
                                return new com.sun.swingfx.binding.ListExpressionHelper.SingleInvalidation<E>(observable, invalidationListeners[0]);
                            ...
                            return new com.sun.swingfx.binding.ListExpressionHelper.SingleChange<E>(observable, changeListeners[1-index]);
                        } else {
                ...
                for (int index = 0; index < listChangeSize; index++) {
                    if (listener.equals(listChangeListeners[index])) {
                        if (listChangeSize == 1) {
                            ...
                            return new com.sun.swingfx.binding.ListExpressionHelper.SingleListChange<E>(observable, listChangeListeners[1-index]);
                        } else {
